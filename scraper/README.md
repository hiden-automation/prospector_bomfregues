...
- Executa uma lista fixa de buscas por nicho e regiao em `index.js`.
- Percorre os resultados organicos do Google Maps.
- Ignora resultados patrocinados.
- Extrai e normaliza telefones brasileiros para o formato com codigo `55`.
- Evita processar o mesmo resultado e o mesmo telefone durante a execucao.
- Envia cada lead para `POST /contacts` da `cadence` com `name`, `phone` e `niche`.
